import { c as bodyLockToggle, d as bodyLockStatus } from "./common.min.js";
(function polyfill() {
...
function gridInit() {
  const items = document.querySelector(".main-portfolio__items");
  if (items) {
    let documentActions = function(e) {
      const targetElement = e.target;
      if (targetElement.closest(".main-portfolio__filter-link")) {
        const filterItem = targetElement.closest(".main-portfolio__filter-link");
        console.log(filterItem);
        const filterValue = filterItem.dataset.filter;
        const filterActiveItem = document.querySelector(".main-portfolio__filter-link.active");
        filterValue === "*" ? iso.arrange({ filter: `` }) : iso.arrange({ filter: `[data-filter="${filterValue}"]` });
    ...
    const iso = new Isotope(items, {
      itemSelector: ".main-portfolio__item",
      percentPosition: true,
      masonry: {
        columnWidth: ".main-portfolio__sizer",
        gutter: 16
      }
    });
